...
const One = () => {
  ...
  useEffect(() => {
    ...
    timeline
      .fromTo('.user-type-background', { opacity: 0, scale: 1.1 }, { opacity: 1, scale: 1, duration: 1.5 })
      .from('.user-type-heading', { opacity: 0, y: 30, duration: 0.5 }, '-=1')
      .fromTo('.user-type-buttons', { opacity: 0, y: 20, stagger: 0.1, duration: 0.3 }, {opacity:1});
  }, []);

  const handleSelect = (type) => {
    if (type === 'client'){
    gsap.to('.user-type-selection', { opacity: 0, duration: 2, onComplete: () => navigate('/ClientSignIn') });
        navigate()
    }
    else{
        gsap.to('.user-type-selection', { opacity: 0, duration: 2, onComplete: () => navigate('/AdvocateSignIn') });
    }
